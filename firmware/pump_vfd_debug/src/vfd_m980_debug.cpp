...
bool VfdM980Debug::read_reg(uint16_t reg, uint16_t* value) {
  ...
  uint16_t tmp = 0;
  if (!modbus_.read_holding_registers(MODBUS_SLAVE_ID, reg, 1, &tmp)) return false;
  *value = tmp;
  return true;
}

bool VfdM980Debug::read_block(uint16_t reg, uint16_t count, uint16_t* out) {
  return modbus_.read_holding_registers(MODBUS_SLAVE_ID, reg, count, out);
}

bool VfdM980Debug::write_reg(uint16_t reg, uint16_t value) {
  return modbus_.write_single_register(MODBUS_SLAVE_ID, reg, value);
}
