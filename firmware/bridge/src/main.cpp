...
static bool cobs_decode(const uint8_t* in, size_t len, uint8_t* out, size_t* out_len) {
...
#pragma pack(push, 1)
struct UsbHdr { uint8_t proto; uint8_t type; uint16_t seq; uint32_t mono_ms; };
...
struct NowCmdFlow {
  ...
  uint8_t flags; // bit1 STOP priority
  uint16_t crc;
};
